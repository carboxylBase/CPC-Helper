...
pub async fn fetch_contests() -> Result<Vec<Contest>> {
    ...
    let re_title_link = Regex::new(r#"contest__title"><a\s+href="(/contest/[^"]+)"[^>]*>([^<]+)</a>"#).unwrap();
    ...
    for chunk in chunks.iter().skip(1) {
        ...
        let start_time_utc: DateTime<Utc> = DateTime::from_timestamp(timestamp_i64, 0)
...
pub async fn fetch_user_stats(handle: &str) -> Result<UserStats> {
    ...
    let solved_count = re_solved.captures(&html)
        ...
        .unwrap_or_else(|| {
            ...
            let all_nums: Vec<u32> = re_any_numbox.captures_iter(&html)
                .map(|c| c[1].parse::<u32>().unwrap_or(0))
                .collect();
            if all_nums.len() >= 2 { all_nums[1] } else { 0 }
        });

    // 2. 匹配最近一次 Rating - 第一个匹配项即为最新
    let re_rating = Regex::new(r#"class="col--new_rating"[^>]*>(\d+)</td>"#).unwrap();
    let latest_rating = re_rating.captures(&html)
        .and_then(|cap| cap[1].parse::<u32>().ok());
